...
class ForcedWNLinear(nn.Module):
    def __init__(self, C_in, C_out, bias = False):
        super().__init__()
        w = torch.randn(C_out, C_in)
        self.weight = nn.Parameter(w)
        self.bias = None if not bias else nn.Parameter(torch.zeros(C_out))

    def forward(self, x):
        if self.training:
            with torch.no_grad():
                self.weight.weight = normalize(self.weight.clone().detach())
...
def snake_beta(x, alpha, beta):
    sin = torch.sin(x * alpha)
    return x + (1.0 / (beta + 0.000000001)) * sin * sin
