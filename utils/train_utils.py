...
def maybe_resume_checkpoint_fine(args, model, optimizer, scheduler, reporter, train_dl):
    ''' 
    for the fine-tuning, we only load the model weights
     but not optimizer the scheduler and reporter.
    '''
    if args.resume is not None and Path(args.resume).is_file():
        checkpoint = args.resume
        logging.info(f"Resume from the provided checkpoitn {args.resume}")
