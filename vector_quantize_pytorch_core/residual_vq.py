...
class MultiScaledResidualVQ(Module):
    ...
    def __init__(
        ...
        **vq_kwargs
        ...
        self.layers = ModuleList([VectorQuantize(dim = codebook_dim, codebook_size = layer_codebook_size, codebook_dim = codebook_dim, 
                       accept_image_fmap = accept_image_fmap, vq_stride=vq_strides[i], **vq_kwargs) for i, layer_codebook_size in enumerate(codebook_sizes)])
    ...
    def forward(
        ...
        #all_losses = map(partial(torch.stack, dim = -1), (all_losses))
        all_losses =  torch.stack(all_losses, dim = -1)  #map(partial(torch.stack, dim = -1), (all_losses))

        ret = (quantized_out, all_indices, all_losses, all_quantized)
