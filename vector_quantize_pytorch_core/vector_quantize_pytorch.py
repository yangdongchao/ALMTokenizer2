...
class VectorQuantize(Module):
    def __init__(
        ...
        """ we add the vq_stride paramter. The default value is 1.
        """
        super().__init__()
        self.dim = dim
        self.heads = heads
    ...
    def forward(
        ...
        # print('x ', x.shape)
        if self.vq_stride > 1:
            x = torch.nn.functional.avg_pool1d(x.transpose(1, 2), self.vq_stride, self.vq_stride)
            x = x.transpose(1, 2)
        orig_input = x
        ...
        if self.vq_stride > 1:
            quantize = quantize.transpose(1, 2).repeat_interleave(self.vq_stride, dim=-1)
